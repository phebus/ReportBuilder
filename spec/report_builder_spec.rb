describe ReportBuilder do
  ...
  it 'correctly combines multiple JSON reports into a single grouped json report' do
  ...
  it 'correctly combines multiple JSON reports into a single grouped HTML report' do
    ...
    options         = {
      json_path:       {
        'Group A' => ["#{TEST_FIXTURES_DIRECTORY}/json_reports/report.json", "#{TEST_FIXTURES_DIRECTORY}/json_reports/report2.json"],
  ...
  it 'names all reports the same by default' do
    ...
    ReportBuilder.build_report(report_types: %i[json html retry], json_path: [input_path], report_path: generic_output_location)
  ...
  it 'can name reports individually' do
    ...
    ReportBuilder.build_report(report_types: %i[json html retry], json_path: [input_path], json_report_path: json_output_location, html_report_path: html_output_location, retry_report_path: retry_output_location)
  ...
  it 'prioritizes specific report names over the general name' do
    ...
    ReportBuilder.build_report(report_types: %i[json html retry], json_path: [input_path], report_path: generic_output_location, html_report_path: html_output_location)
    ...
    context 'with specific configuration' do
      ...
      before(:each) do
        ReportBuilder.configure do |configuration|
          ...
          configuration[:report_types] = %i[json retry]
        end
      end
